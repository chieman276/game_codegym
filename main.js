...
canvas.addEventListener("click", function (event) {
    if (!isEggFlying) {
        ...
        sound.play();
        sound.play();
        // Xoay quả trứng 360 độ
        rotateEgg();

    }
});
    // Hàm xoay quả trứng 360 độ
    function rotateEgg() {
        var rotationAngle = 0;
        var rotationSpeed = 30; // Tốc độ xoay
        
        // Hàm thực hiện xoay quả trứng
        function rotate() {
        context.clearRect(eggX, eggY, quatrung.width, quatrung.height); // Xóa vùng vẽ cũ
        context.save();
        context.translate(eggX + quatrung.width / 2, eggY + quatrung.height / 2); // Đặt tâm xoay ở giữa quả trứng
        context.rotate((rotationAngle * Math.PI) / 180); // Xoay theo góc rotationAngle
        context.drawImage(quatrung, -quatrung.width / 2, -quatrung.height / 2); // Vẽ quả trứng đã xoay
        context.restore();
        rotationAngle += rotationSpeed; // Tăng góc xoay
        
        // Kiểm tra nếu đã xoay đủ 360 độ thì dừng
        if (rotationAngle >= 960) {
            rotationAngle = 0;
            return;
        }
